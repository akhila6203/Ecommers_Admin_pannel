...
export const getCoupon = async (req, res) => {
  try {
    const coupons = await query(`SELECT ${COUPON_COLUMNS} FROM coupons WHERE id = ?`, [req.params.id]);
...
export const createCoupon = async (req, res) => {
  try {
    ...
    const existing = await query("SELECT id FROM coupons WHERE code = ?", [code.toUpperCase()]);
    if (existing.length) return errorResponse(res, "Coupon code already exists", 409);

    const result = await query(
      `INSERT INTO coupons (code, type, value, minimum_order_amount, maximum_discount, usage_limit, used_count, per_user_limit, is_for_new_customers, start_date, expiry_date, status, description)
       VALUES (?, ?, ?, ?, ?, ?, 0, ?, ?, ?, ?, ?, ?)`,
      [
        code.toUpperCase(),
        type || "percentage",
    ...
    const coupon = await query(`SELECT ${COUPON_COLUMNS} FROM coupons WHERE id = ?`, [result.insertId]);
...
export const updateCoupon = async (req, res) => {
  try {
    ...
    const existing = await query(`SELECT ${COUPON_COLUMNS} FROM coupons WHERE id = ?`, [req.params.id]);
    ...
    if (nextCode !== existing[0].code) {
      const dup = await query("SELECT id FROM coupons WHERE code = ? AND id != ?", [nextCode, req.params.id]);
    ...
    const coupon = await query(`SELECT ${COUPON_COLUMNS} FROM coupons WHERE id = ?`, [req.params.id]);
...
export const getCouponUsage = async (req, res) => {
  try {
    ...
    const coupon = await query("SELECT id, code FROM coupons WHERE id = ?", [couponId]);
    ...
    const usage = await query(
      `SELECT cu.id, cu.coupon_id, cu.order_id, cu.discount_amount, cu.used_at,
       ...
       LIMIT ? OFFSET ?`,
      [couponId, String(limit), String(offset)]
    );
...
// @desc    Get all coupon usage (admin overview)
// @route   GET /api/coupons/usage/all
export const getAllCouponUsage = async (req, res) => {
  try {
    ...
    const countResult = await query("SELECT COUNT(*) as total FROM coupon_usage");
    const total = countResult[0].total;
...
export const recordCouponUsage = async (connection, { couponCode, customerId, orderId, discountAmount }) => {
  ...
  await connection.query(
    ...
    [couponId, customerId || null, orderId || null, discountAmount || 0]
  );
  await connection.query(
